    ...
    <div className="flex gap-4 overflow-x-auto pb-6 pt-2 items-start min-h-[calc(100vh-220px)]">
      {STAGES_CONFIG.map((stage) => {
        ...
        return (
          <div
            key={stage.id}
            className={`w-72 shrink-0 rounded-xl border border-slate-200 bg-slate-100/70 flex flex-col max-h-[calc(100vh-230px)] shadow-xs ${stage.color}`}
          >
            {/* Stage Column Header */}
            <div className="p-3 border-b border-slate-200/80 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="h-5 min-w-[20px] px-1.5 rounded-full bg-white border border-slate-200 text-slate-600 text-[10px] font-bold flex items-center justify-center">
